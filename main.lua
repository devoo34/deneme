...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player ~= nil then
        ...
        MySQL.Async.execute(false, "SELECT * FROM player_contacts WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ORDER BY `name` ASC", function(result)
            local Contacts = {}
            ...
            MySQL.Async.execute(false, "SELECT * FROM billing WHERE `identifier` = '"..Player.PlayerData.citizenid.."'", function(invoices)
                if invoices[1] ~= nil then
                    for k, v in pairs(invoices) do
                        ...
                        if Ply ~= nil then
                            ...
                            MySQL.Async.execute(true, "SELECT * FROM `players` WHERE `citizenid` = '"..v.sender.."'", function(res)
                                if res[1] ~= nil then
                ...
                MySQL.Async.execute(false, "SELECT * FROM player_vehicles WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(garageresult)
                    ...
                    MySQL.Async.execute(false, "SELECT * FROM phone_messages WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ORDER BY edited_at DESC", function(messages)
                        ...
                        MySQL.Async.execute(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
                            if mails[1] ~= nil then
                            ...
                            MySQL.Async.execute(false, 'SELECT * FROM `crypto_transactions` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(transactions)
                                ...
                                MySQL.Async.execute(false, 'SELECT * FROM `phone_photos` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `created_at` DESC', function(photos)
...
AddEventHandler('qb-phone:server:RemoveMail', function(MailId)
    ...
    MySQL.Async.execute(false, 'DELETE FROM `player_mails` WHERE `mailid` = "'..MailId..'" AND `citizenid` = "'..Player.PlayerData.citizenid..'"')
...
AddEventHandler('qb-phone:server:sendNewMail', function(mailData)
    ...
    if mailData.button == nil then
        MySQL.Async.execute(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..Player.PlayerData.citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0')")
    else
        MySQL.Async.execute(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..Player.PlayerData.citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')")
    end
    TriggerClientEvent('qb-phone:client:NewMailNotify', src, mailData)
    SetTimeout(200, function()
        MySQL.Async.execute(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` DESC', function(mails)
            if mails[1] ~= nil then
                for k, v in pairs(mails) do
                    if mails[k].button ~= nil then
                        mails[k].button = json.decode(mails[k].button)
...
AddEventHandler('qb-phone:server:sendNewMailToOffline', function(citizenid, mailData)
    ...
    if Player ~= nil then
        ...
        if mailData.button == nil then
            MySQL.Async.execute(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0')")
        else
            MySQL.Async.execute(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')")
        end
    end
end)
...
AddEventHandler('qb-phone:server:ClearButtonData', function(mailId)
    ...
    MySQL.Async.execute(false, 'UPDATE `player_mails` SET `button` = "" WHERE `mailid` = "'..mailId..'" AND `citizenid` = "'..Player.PlayerData.citizenid..'"')
    SetTimeout(200, function()
        MySQL.Async.execute(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` DESC', function(mails)
            if mails[1] ~= nil then
                for k, v in pairs(mails) do
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..firstName.."%' AND `charinfo` LIKE '%"..lastName.."%'", function(result)
                    if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    ...
    if Trgt ~= nil then
        if Ply.PlayerData.money.bank >= amount then
            ...
            MySQL.Async.execute(true, "DELETE FROM `billing` WHERE `id` = '"..invoiceId.."'")
            MySQL.Async.execute(false, "SELECT * FROM `billing` WHERE `identifier` = '"..Ply.PlayerData.citizenid.."'", function(invoices)
                if invoices[1] ~= nil then
                    for k, v in pairs(invoices) do
        ...
        MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `citizenid` = '"..sender.."'", function(result)
            if result[1] ~= nil then
                local moneyInfo = json.decode(result[1].money)
                moneyInfo.bank = math.ceil((moneyInfo.bank + amount))
                MySQL.Async.execute(true, "UPDATE `players` SET `money` = '"..json.encode(moneyInfo).."' WHERE `citizenid` = '"..sender.."'")
                Ply.Functions.RemoveMoney('bank', amount, "paid-invoice")
                MySQL.Async.execute(true, "DELETE FROM `phone_invoices` WHERE `invoiceid` = '"..invoiceId.."'")
                MySQL.Async.execute(false, "SELECT * FROM `phone_invoices` WHERE `citizenid` = '"..Ply.PlayerData.citizenid.."'", function(invoices)
                    if invoices[1] ~= nil then
                        for k, v in pairs(invoices) do
                            local Target = QBCore.Functions.GetPlayerByCitizenId(v.sender)
                            if Target ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        ...
        MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..v.number.."%'", function(result)
            if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone:server:GetContactPicture', function(source, cb, Chat)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..Chat.number.."%'", function(result)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetPicture', function(source, cb, number)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..number.."%'", function(result)
        if result[1] ~= nil then
            local MetaData = json.decode(result[1].metadata)
...
AddEventHandler('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                MySQL.Async.execute(false, "UPDATE `players` SET `money` = '"..json.encode(moneyInfo).."' WHERE `citizenid` = '"..result[1].citizenid.."'")
...
AddEventHandler('qb-phone:server:EditContact', function(newName, newNumber, newIban, newImage, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute(false, "UPDATE `player_contacts` SET `name` = '"..newName.."', `number` = '"..newNumber.."', `iban` = '"..newIban.."', `image` = '"..newImage.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `name` = '"..oldName.."' AND `number` = '"..oldNumber.."'")
end)
...
AddEventHandler('qb-phone:server:EditContactNote', function(name, number, newNote)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute(false, "UPDATE `player_contacts` SET `note` = '"..newNote.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `name` = '"..name.."' AND `number` = '"..number.."'")
end)
...
AddEventHandler('qb-phone:server:RemoveContact', function(Name, Number)
    ...
    MySQL.Async.execute(false, "DELETE FROM `player_contacts` WHERE `name` = '"..Name.."' AND `number` = '"..Number.."' AND `citizenid` = '"..Player.PlayerData.citizenid.."'")
end)
...
AddEventHandler('qb-phone:server:AddNewContact', function(name, number, iban, image)
    ...
    MySQL.Async.execute(false, "INSERT INTO `player_contacts` (`citizenid`, `name`, `number`, `iban`, `image`) VALUES ('"..Player.PlayerData.citizenid.."', '"..tostring(name).."', '"..tostring(number).."', '"..tostring(iban).."', '"..tostring(image).."')")
end)
...
AddEventHandler('qb-phone:server:SendMessageToPlayer', function(SenderNumber, TargetNumber, Message, MessageType, MessageMetaData)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..TargetNumber.."%'", function(data)
...
AddEventHandler('qb-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..ChatNumber.."%'", function(Player)
        if Player[1] ~= nil then
            local TargetData = QBCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)

            if TargetData ~= nil then
                MySQL.Async.execute(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."' ORDER BY edited_at DESC", function(Chat)
                    if Chat[1] ~= nil then
                        -- Update for target
                        MySQL.Async.execute(false, "UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..TargetData.PlayerData.citizenid.."' AND `number` = '"..SenderData.PlayerData.charinfo.phone.."'")
                                
                        -- Update for sender
                        MySQL.Async.execute(false, "UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..TargetData.PlayerData.charinfo.phone.."'")
                        ...
                        -- Insert for target
                        MySQL.Async.execute(false, "INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..TargetData.PlayerData.citizenid.."', '"..SenderData.PlayerData.charinfo.phone.."', '"..json.encode(ChatMessages).."')")
                                            
                        -- Insert for sender
                        MySQL.Async.execute(false, "INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..SenderData.PlayerData.citizenid.."', '"..TargetData.PlayerData.charinfo.phone.."', '"..json.encode(ChatMessages).."')")
                ...
                MySQL.Async.execute(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."' ORDER BY edited_at DESC", function(Chat)
                    if Chat[1] ~= nil then
                        -- Update for target
                        MySQL.Async.execute(false, "UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..Player[1].citizenid.."' AND `number` = '"..SenderData.PlayerData.charinfo.phone.."'")
                                
                        -- Update for sender
                        Player[1].charinfo = json.decode(Player[1].charinfo)
                        MySQL.Async.execute(false, "UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..Player[1].charinfo.phone.."'")
                    else
                        -- Insert for target
                        MySQL.Async.execute(false, "INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..Player[1].citizenid.."', '"..SenderData.PlayerData.charinfo.phone.."', '"..json.encode(ChatMessages).."')")
                        
                        -- Insert for sender
                        Player[1].charinfo = json.decode(Player[1].charinfo)
                        MySQL.Async.execute(false, "INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..SenderData.PlayerData.citizenid.."', '"..Player[1].charinfo.phone.."', '"..json.encode(ChatMessages).."')")
                    end
                end)
            end
        else
            ...
            MySQL.Async.execute(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."' ORDER BY edited_at DESC", function(Chat)
                if Chat[1] ~= nil then
                    -- Update for sender
                    MySQL.Async.execute(false, "UPDATE `phone_messages` SET `messages` = '"..json.encode(ChatMessages).."' WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'")
                else
                    -- Insert for sender
                    MySQL.Async.execute(false, "INSERT INTO `phone_messages` (`citizenid`, `number`, `messages`) VALUES ('"..SenderData.PlayerData.citizenid.."', '"..ChatNumber.."', '"..json.encode(ChatMessages).."')")
                end
            end)
        end
    end)
...
AddEventHandler('qb-phone:server:SaveMetaData', function(MData)
    ...
    MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
        local MetaData = json.decode(result[1].metadata)
        MetaData.phone = MData
        MySQL.Async.execute(false, "UPDATE `players` SET `metadata` = '"..json.encode(MetaData).."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")
    end)
...
QBCore.Functions.CreateCallback('qb-phone:server:FetchResult', function(source, cb, search)
    ...
    MySQL.Async.execute(false, query, function(result)
        MySQL.Async.execute(false, 'SELECT * FROM `apartments`', function(ApartmentData)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetVehicleSearchResults', function(source, cb, search)
    ...
    local search = escape_sqli(search)
    local searchData = {}
    MySQL.Async.execute(false, 'SELECT * FROM `player_vehicles` WHERE `plate` LIKE "%'..search..'%" OR `citizenid` = "'..search..'"', function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                MySQL.Async.execute(true, 'SELECT * FROM `players` WHERE `citizenid` = "'..result[k].citizenid..'"', function(player)
                    if player[1] ~= nil then 
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    local vehicleData = {}
    if plate ~= nil then 
        MySQL.Async.execute(false, 'SELECT * FROM `player_vehicles` WHERE `plate` = "'..plate..'"', function(result)
            if result[1] ~= nil then
                MySQL.Async.execute(true, 'SELECT * FROM `players` WHERE `citizenid` = "'..result[1].citizenid..'"', function(player)
                    local charinfo = json.decode(player[1].charinfo)
                    vehicleData = {
                        plate = plate,
                        status = true,
        ...
        MySQL.Async.execute(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
            if result[1] ~= nil then
                ...
                if Reciever ~= nil then
                    ...
                    MySQL.Async.execute(false, "UPDATE `players` SET `money` = '"..json.encode(RecieverMoney).."' WHERE `citizenid` = '"..result[1].citizenid.."'")
                end
                cb(true)
            else
                TriggerClientEvent('QBCore:Notify', src, "Böyle bir hesap yok", "error")
...
AddEventHandler('qb-phone:server:AddTransaction', function(data)
    ...
    MySQL.Async.execute(false, "INSERT INTO `crypto_transactions` (`citizenid`, `title`, `message`) VALUES ('"..Player.PlayerData.citizenid.."', '"..escape_sqli(data.TransactionTitle).."', '"..escape_sqli(data.TransactionMessage).."')")
end)
...
AddEventHandler('qb-phone:server:DeletePhoto', function(id)
    local player = QBCore.Functions.GetPlayer(source)
    MySQL.Async.execute(false, "DELETE FROM `phone_photos` WHERE citizenid='"..player.PlayerData.citizenid.."' AND id="..id)
end)